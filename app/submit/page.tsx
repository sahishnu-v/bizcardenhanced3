...
// Submissions land as status='pending', trigger admin email via /api/notify
"use client";
...
export default function SubmitPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (insertError || !card) {
        toastError(`Submission failed: ${insertError?.message ?? "Unknown error"}`);
        setSubmitting(false);
        return;
      }
      ...
      if (photoFile) {
        ...
        if (uploadError) {
          // Card saved, photo failed — not fatal
            ...
            {[
              ...
              <div key={name} className={span === 2 ? "sm:col-span-2" : ""}>
                ...
                <input
                  ...
                  className="w-full text-sm border border-slate-200 rounded-lg px-3 py-2.5 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-shadow"
                />
              </div>
            ))}
          </div>
