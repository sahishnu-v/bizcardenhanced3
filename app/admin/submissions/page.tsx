...
const EMPTY_FORM = {
  full_name: "", title: "", company: "", email: "", phone: "", website: "", category_id: "", bio: "",
};
...
// --- mirror-div autogrow Bio textarea -----------------------------------
// The mirror <div> sits in normal flow and dictates the wrapper's height.
// The <textarea> is absolutely positioned on top, filling the wrapper.
// As `value` grows, the mirror grows, the wrapper grows, the textarea grows
// with it — no scrollHeight reads, no useEffect, no jitter.
function BioField({
  value,
  onChange,
  disabled,
  ...
  // Trailing space makes a final newline produce a visible line in the mirror.
  const mirrorText = (value || placeholder || "") + " ";
  // Shared typography MUST match between mirror and textarea.
  const sharedClass =
    "w-full text-sm leading-relaxed px-3 py-2 box-border whitespace-pre-wrap break-words";
  return (
    <div className="relative w-full">
      <div
        aria-hidden
        className={`${sharedClass} invisible pointer-events-none border border-transparent`}
      ...
      <textarea
        ...
        rows={1}
        className={`${sharedClass} absolute inset-0 resize-none overflow-hidden bg-white text-slate-900 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none`}
      />
    </div>
  );
}

// --- BioBlock: 3-line clamp + per-card more/less toggle ------------------
function BioBlock({
  text,
  expanded,
  onToggle,
  ...
  // Heuristic: only render the toggle when the bio is plausibly clamped.
  // ~140 chars roughly corresponds to 3 lines at this column width.
  const mayOverflow = text.length > 140;
  return (
    <div className="mt-3">
      <p
      ...
      {mayOverflow && (
        <button
          type="button"
          onClick={onToggle}
          className="mt-1 text-[11px] font-semibold text-blue-600 hover:text-blue-800 underline"
        >
          {expanded ? "less" : "... more"}
        </button>
      )}
...
export default function Page() {
  ...
  // Per-card expanded state for the bio more/less toggle
  const [expandedBios, setExpandedBios] = useState<Record<string, boolean>>({});

  // Streaming flags for the in-form Generate Bio button (Add and Edit)
  const [addBioStreaming, setAddBioStreaming] = useState(false);
  const [editBioStreaming, setEditBioStreaming] = useState(false);
  ...
  // ── Generate bio into the Add/Edit form ──────────────────
  const streamBioIntoForm = async (
    payload: { name: string; title: string; business: string; category?: string },
              ...
              <div className="sm:col-span-2">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      const cat = categories.find(c => c.id === addFormData.category_id)?.name;
                      streamBioIntoForm(
                        { name: addFormData.full_name, title: addFormData.title, business: addFormData.company, category: cat },
                        full => setAddFormData((prev: any) => ({ ...prev, bio: full })),
                        () => { setAddBioStreaming(true); setAddFormData((prev: any) => ({ ...prev, bio: "" })); },
                        () => setAddBioStreaming(false),
                      );
                    }}
                    disabled={
                      addBioStreaming ||
                      !addFormData.full_name.trim() ||
                      !addFormData.title.trim() ||
                      !addFormData.company.trim()
                    }
                    className="text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded border border-purple-100 bg-purple-50 text-purple-600 hover:bg-purple-500 hover:text-white transition-colors disabled:opacity-40 disabled:hover:bg-purple-50 disabled:hover:text-purple-600"
                  >
                    {addBioStreaming ? "Writing…" : "✦ Generate Bio"}
                  </button>
                </div>
                <BioField
                  value={addFormData.bio ?? ""}
                  onChange={v => setAddFormData({ ...addFormData, bio: v })}
                  disabled={addBioStreaming}
                  placeholder="A short, friendly bio for this card…"
                />
              </div>
            </div>
                ...
                <div className="p-6 flex flex-col h-full">
                  ...
                  {isEditing ? (
                      ...
                      <div className="pt-1">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <button
                            type="button"
                            onClick={() => {
                              const cat = categories.find(c => c.id === editFormData.category_id)?.name;
                              streamBioIntoForm(
                                { name: editFormData.full_name, title: editFormData.title, business: editFormData.company, category: cat },
                                full => setEditFormData((prev: any) => ({ ...prev, bio: full })),
                                () => { setEditBioStreaming(true); setEditFormData((prev: any) => ({ ...prev, bio: "" })); },
                                () => setEditBioStreaming(false),
                              );
                            }}
                            disabled={
                              editBioStreaming ||
                              !editFormData.full_name?.trim() ||
                              !editFormData.title?.trim() ||
                              !editFormData.company?.trim()
                            }
                            className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded border border-purple-100 bg-purple-50 text-purple-600 hover:bg-purple-500 hover:text-white transition-colors disabled:opacity-40 disabled:hover:bg-purple-50 disabled:hover:text-purple-600"
                          >
                            {editBioStreaming ? "Writing…" : "✦ Generate"}
                          </button>
                        </div>
                        <BioField
                          value={editFormData.bio ?? ""}
                          onChange={v => setEditFormData({ ...editFormData, bio: v })}
                          disabled={editBioStreaming}
                          placeholder="A short bio for this card…"
                        />
                      </div>
                    ...
                    <div className="mb-4">
                      ...
                      {/* AI Bio — show streaming bio if present, else the persisted one */}
                      {(() => {
                        const displayBio = bio ?? card.bio ?? "";
                        if (!displayBio && !isGenerating) return null;
                        // While streaming, render plain with a cursor (no clamp/toggle yet)
                        if (isGenerating) {
                          return (
                            <p className="mt-3 text-xs text-slate-500 leading-relaxed italic" style={{ whiteSpace: "pre-wrap" }}>
                              {displayBio}
                        ...
                        return (
                          <BioBlock
                            text={displayBio}
                            expanded={!!expandedBios[card.id]}
                            onToggle={() => setExpandedBios(prev => ({ ...prev, [card.id]: !prev[card.id] }))}
                          />
                        );
                      })()}
